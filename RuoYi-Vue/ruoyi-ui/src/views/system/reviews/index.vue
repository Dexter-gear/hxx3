    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:reviews:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:reviews:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:reviews:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listReviews, getReviews, delReviews, addReviews, updateReviews } from "@/api/system/reviews";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listReviews(this.queryParams).then(response => {
        this.reviewsList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getReviews(reviewId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.reviewId != null) {
            updateReviews(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addReviews(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除评论详情编号为"' + reviewIds + '"的数据项？').then(function() {
        return delReviews(reviewIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/reviews/export', {
        ...this.queryParams
